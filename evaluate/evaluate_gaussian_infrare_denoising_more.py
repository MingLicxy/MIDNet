...
for dataset in datasets:
    ...
    for sigma_test in sigmas:
        ...
        file_path = os.path.join('/home/caoxinyu/UNet-based/Xformer-main/results/Uformer', dataset, str(sigma_test))
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
            for filename, PSNR_SSIM in zip(img_files, executor.map(proc, img_files)):
                ...
                clip_iqa.append(PSNR_SSIM[2])

        avg_psnr = sum(psnr)/len(psnr)
        avg_ssim = sum(ssim)/len(ssim)
        avg_clip_iqa = sum(clip_iqa)/len(clip_iqa)
        ...
        print('For {:s} dataset CLIP-IQA: {:f}\n'.format(dataset, avg_clip_iqa))
