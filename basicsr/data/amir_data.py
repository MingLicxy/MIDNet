import torch
import os
import SimpleITK as sitk #TODO 专注于医学图像处理的库
import pickle
# import pydicom
import numpy as np
import datetime
...
class transformPET:
    ...
    def denormalize(self, img):
        ...
        img = self.clip(img)  # Ensure the image is clipped to a valid range
        return img
...
class dataIO:
    ...
    # 从ITK图像文件加载数据，sitk.ReadImage读取文件
    # sitk.GetArrayFromImage将其转换为NumPy数组
    def load_itk(self, path):
        return sitk.GetArrayFromImage(sitk.ReadImage(path))

    #TODO .bin文件的读取结果类型取决于存储类型
    ...
    # Add PNG save and load methods
    def save_png(self, data, path):
        if isinstance(data, np.ndarray):  # Ensure data is in the right format
            image = Image.fromarray(data.astype('uint8'))
            image.save(path)
        else:
            raise TypeError("Data must be a numpy array to save as PNG.")
        
    # 从PNG文件加载图像，并转换为NumPy数组。
