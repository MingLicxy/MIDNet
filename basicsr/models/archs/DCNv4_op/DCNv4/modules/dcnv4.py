...
class DCNv4(nn.Module):
    def __init__(
        ...
        #BUG 当采用DCNv4实现DWConv时，_d_per_group=1（DCNv4目前要求每组通道数是8的倍数）
        assert _d_per_group % 16 == 0 or _d_per_group == 1
    ...
    def forward(self, input, shape=None):
        ...
        """
        #print("################################", input.shape) #[1, 96, 128, 128]
        #TODO # (N, C, H, W) -> (N, L=H * W, C)
        N, C, H, W = input.shape 
