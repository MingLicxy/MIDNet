...
from timm.layers import to_2tuple, trunc_normal_, LayerNorm2d

#TODO 引入基于INR的低通滤波器
#from liif_inr import INR 
from .liif_inr import INR
#from .srno_inr import INR
...
class SpatialTransformerBlock(nn.Module):
    def __init__(self,
        ...
        # self.norm1 = DynamicTanh(dim, True)
        ...
        # self.norm2 = DynamicTanh(dim, True)
...
class INRDSUNet(nn.Module):
    def __init__(self, 
        ...
        inr_act = 'relu',  # 'relu', 'gelu', 'sine', 'expsin', 'gaussian', 'quadratic', 'multi_quadratic', 'laplacian', 'super_gaussian'
        inr_hidden_list = [[256], [256, 256], [256, 256, 256]]  # Hidden layer sizes for INR
    ):

        super(INRDSUNet, self).__init__()
        self.alpha = 1
        self.beta = 1

        
        #########################################TODO INR #########################################
        # ["relu", "gelu", 'sine', 'expsin', 'gaussian', 'quadratic', 'multi_quadratic', 'laplacian', 'super_gaussian']
        ### INR 123  主要思路

        self.INR1 = INR(dim * 4, inr_hidden_list[0], inr_act).cuda()
        self.INR2 = INR(dim * 2, inr_hidden_list[1], inr_act).cuda()
        self.INR3 = INR(dim * 1, inr_hidden_list[2], inr_act).cuda()

        ### INR 321  消融思路
        # self.INR1 = INR(dim * 4, [256, 256, 256], "gelu").cuda()
        # self.INR2 = INR(dim * 2, [256, 256], "gelu").cuda()
        # self.INR3 = INR(dim * 1, [256], "gelu").cuda()
        #########################################  end  #####################################
        ...
        self.refinement = nn.Sequential(*[SpatialTransformerBlock(dim=int(dim), input_resolution=(img_size, img_size),
                             num_heads=heads[0], window_size=window_size[0], shift_size=0 if (i % 2 == 0) else window_size[0] // 2,
                             mlp_ratio=ffn_expansion_factor,
                             layer_idx=i+1,
                             drop_path=0.) for i in range(num_refinement_blocks)])
    ...
    def forward(self, inp_img):
        ...
        ## 第一次残差链接（从下到上）
        out_enc3_inr = self.INR1(out_enc3)
        inp_dec3 = torch.cat([inp_dec3, out_enc3_inr], 1)
        ...
        ## 第二次残差链接
        out_enc2_inr = self.INR2(out_enc2)
        inp_dec2 = torch.cat([inp_dec2, out_enc2_inr], 1)
        ...
        ## 第三次残差链接
        out_enc1_inr = self.INR3(out_enc1)
        inp_dec1 = torch.cat([inp_dec1, out_enc1_inr], 1)
...
if __name__== '__main__':
    ...
    model = INRDSUNet().cuda()
    inputs = torch.randn(1, 1, 128, 128).cuda()  # 适配你的输入尺寸
